      ...
      <View style={styles.row3}>
        {player.office === 'Fiscal' ?
          <>
            ...
            <View>
              <Text style={[styles.text, { marginTop: 5 }]}>{item.type === 'stamp' ? `Parcela ${item.idParcel + 1}` : `$${normalizeNumber(item.value)}`} </Text>
          ...
          <>
            ...
            <View>
              <Text style={[styles.text, { marginTop: 5 }]}>{item.type === 'prevention' ? `$${normalizeNumber(item.value)}` : `${item.label}`} </Text>
