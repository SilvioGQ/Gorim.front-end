...
export default function Vendas({ navigation, route }) {
  ...
  const filterPlayers = () => {
    ...
    p.unshift({ name: 'Todos', avatar: 'Todos', id: -1, city: "" });
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.alignQuadrado}>
          {filterPlayers().map((item) => <Quadrados city={item.city}abr={false} key={item.id} player={item} onClick={() => setSelectClient(item.id)} backgroundColor={selectClient == item.id ? '#8ACF3A' : '#fff'} color={selectClient == item.id ? '#fff' : '#000'} />)}
        </View>
        {modalText !== '' && (
