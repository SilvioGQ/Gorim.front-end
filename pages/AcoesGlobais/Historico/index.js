...
export default function Historico({ navigation }) {
	...
	const { player, game, oldLogs, stage } = useContext(GameContext);
							...
							{player.type === 'Agricultor' ?
								<><View style={styles.backgreen}>
									...
									<View style={{ display: open ? 'flex' : 'none', flexDirection: 'column' }}>
										{playerLog.logs && playerLog.logs.filter((item) => item.type == 'plantation').length > 0 ? playerLog.logs.filter((item) => item.type == 'plantation').map((item, index) => {
									...
									<View style={styles.backgreen}>
										...
										<View style={{ display: open7 ? 'flex' : 'none' }}>
											{playerLog.logs && playerLog.logs.filter((item) => item.type == 'stamp').length > 0 ? playerLog.logs.filter((item) => item.type == 'stamp').map((item, index) => {
							...
							{player.office === 'Fiscal' ?
								<>
									<View style={styles.backgreen}>
										...
										<View style={{ display: open3 ? 'flex' : 'none' }}>
											{playerLog.logsOffice && playerLog.logsOffice.filter((item) => item.type == 'stamp').length > 0 ? playerLog.logsOffice.filter((item) => item.type == 'stamp').map((item, index) => {
								...
								<>
									{player.office === "Prefeito" && (
										<View style={styles.backgreen}>
											...
											<View style={{ display: open ? 'flex' : 'none' }}>
												{playerLog.logsOffice && playerLog.logsOffice.filter((item) => {
									...
									<View style={styles.backgreen}>
										...
										{player.office === "Vereador" ?
											...
											:
											<View style={{ display: open5 ? 'flex' : 'none' }}>
												{playerLog.logsOffice && playerLog.logsOffice.filter((item) => item.type == 'prevention').length > 0 ? playerLog.logsOffice.filter((item) => item.type == 'prevention').map((item, index) => {
