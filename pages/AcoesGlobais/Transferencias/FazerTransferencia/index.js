            ...
            <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} >
              {
                stage === 'GETPLAYERS' ?
                  Players().map((item, index) => {
                    return (
                      <Quadrados city={item.city}key={index} abr={item?.type.slice(0, 3)}  player={item} onClick={() => { setId(item.id); }} backgroundColor={id == item.id ? '#8ACF3A' : '#fff'} color={id == item.id ? '#fff' : '#000'} />
                    );
                  })
                  :
                  null
            ...
            <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} >
              {
                stage === 'GETPLAYERS' ?
                  PlayersOffice().map((item, index) => {
                    return (
                      <Quadrados city={item.city}key={index} abr={item?.office.slice(0, 3)}  player={item} onClick={() => { setId(item.id); }} backgroundColor={id == item.id ? '#8ACF3A' : '#fff'} color={id == item.id ? '#fff' : '#000'} />
                    );
                  })
                  :
                  null
