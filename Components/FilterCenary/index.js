		...
		<View style={{ flexDirection: 'row', marginHorizontal: 20, width: '80%', justifyContent: 'space-between', marginVertical: 10 }}>
			...
			{player.office === 'Fiscal' && phase === 2 && (
				<>
					...
					<TouchableOpacity style={[styles.buttonAgr, { backgroundColor: type == 'stamp' ? "#8ACF3A" : '#fff' }]} onPress={() => { setType('stamp') }}  >
						<Text style={[styles.textSmall, { color: type == 'stamp' ? '#fff' : '#000' }]}>Selos</Text>
					</TouchableOpacity>
					<TouchableOpacity style={[styles.buttonAgr, { backgroundColor: type == 'fine' ? "#8ACF3A" : '#fff' }]} onPress={() => { setType('fine') }}  >
			...
			{player.office === 'Prefeito' && phase === 2 && (
				<>
					...
					<TouchableOpacity style={[styles.buttonAgr, { backgroundColor: type == 'tax' ? "#8ACF3A" : '#fff' }]} onPress={() => { setType('tax') }}  >
						<Text style={[styles.textSmall, { color: type == 'tax' ? '#fff' : '#000' }]}>Imposto</Text>
					</TouchableOpacity>
					<TouchableOpacity style={[styles.buttonAgr, { backgroundColor: type == 'prevention' ? "#8ACF3A" : '#fff' }]} onPress={() => { setType('prevention') }}  >
